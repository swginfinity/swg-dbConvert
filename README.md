...
If you're using the `w101` branch of `swginfinity/infinity2.0.0`, `dbconvert.cpp` and the CMake target are already committed to the server repo.

**You still need to apply the engine3 patches and autogen patch before building:**

```bash
# 1. Apply engine3 patches (from swgemu-dbconvert repo)
cd MMOCoreORB/utils/engine3
...
# 2. Patch autogenerated readObject() methods
python3 /path/to/swgemu-dbconvert/scripts/patch_autogen.py MMOCoreORB/src

# 3. Build dbconvert (separate target — NOT included in make all)
cd MMOCoreORB/build
cmake ..
make dbconvert -j24

# 4. Revert engine3 patches (only needed during compilation)
cd MMOCoreORB/utils/engine3
git checkout -- .
...
Alternatively, use `build.sh` which automates steps 1-4 (patches, builds, reverts):
