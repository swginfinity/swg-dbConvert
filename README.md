...
> ### Before You Start
>
> This tool requires your Core3 **and** engine3 to be fully up to date. If you're behind on either, **stop here and update first.**
>
> 1. **Update engine3** — `cd MMOCoreORB/utils/engine3 && git pull`
> 2. **Update Core3** — `git pull` your server repo to get the latest code
> 3. **Apply the [16 required Core3 source changes](docs/CORE3_CHANGES.md)** — these are permanent additions to your server code that dbconvert needs to function correctly
> 4. **Rebuild your server** — `make -j$(nproc)` to make sure everything compiles cleanly
>
> The build script checks your engine3 commit automatically and will refuse to build if it's too old. But if your Core3 is missing the required source changes, dbconvert will either crash or **silently destroy your data** — skills, abilities, template CRCs, and zone names can all be permanently lost with no errors during conversion.
>
> **Do not skip these steps.** There is no recovery if you run a conversion with an incomplete setup.
...
```bash
# 1. Make sure Core3 and engine3 are up to date (see "Before You Start" above)
cd /path/to/MMOCoreORB/utils/engine3 && git pull
cd /path/to/MMOCoreORB && git pull
# Apply the 16 Core3 source changes if you haven't already (see docs/CORE3_CHANGES.md)
make -j$(nproc)
...
1. **Checks engine3 is up to date** — refuses to build if it's too old
2. Copies `dbconvert.cpp` into the server source tree
3. Applies 4 engine3 patches for standalone mode
4. Patches ~325 autogenerated `readObject()` methods
5. Adds the dbconvert CMake target
6. Builds with `cmake -DCMAKE_BUILD_TYPE=RelWithDebInfo .. && make dbconvert`
7. **Reverts all patches and removes `dbconvert.cpp` on exit** (even if the build fails)
...
- **Core3 and engine3 must be up to date** — `git pull` both before building dbconvert. The build script checks engine3 automatically; if your engine3 is too old, the build will fail with instructions to update. If your Core3 is missing the [16 required source changes](docs/CORE3_CHANGES.md), conversion will silently destroy data.
